...
감독이 시나리오에 생각한 **유명 배우 이름을 입력**하고, 지원받은 배우들의 사진을 업로드하면 **해당 배우와의 얼굴 유사도 기준으로 지원자를 자동 랭킹**해주는 AI 기반 캐스팅 매칭 서비스입니다.
...
- 🎯 **레퍼런스 배우 기반 랭킹**: 레퍼런스 배우와의 유사도 순으로 지원자 자동 정렬
- 🔮 **AI 얼굴 인식**: InsightFace AuraFace-v1 모델 기반 (512차원 임베딩)
- 📊 **302명 배우 데이터베이스**: 한국 영화/드라마 주요 배우 내장
...
#### 모드 1: 레퍼런스 배우 기반 지원자 랭킹 (추천)
1. **레퍼런스 배우 입력**: 찾고 싶은 유명 배우 이름 입력 (예: "고윤정", "송강호")
2. **지원자 사진 업로드**: 여러 지원자들의 사진을 드래그&드롭 또는 파일 선택
3. **분석 시작**: "🪔 지니의 마법 시작 ✨" 버튼 클릭
4. **결과 확인**: 
   - **레퍼런스 배우와의 유사도 순으로 지원자 자동 정렬**
   - 1위부터 순위 표시
   - 각 지원자의 유사도 퍼센트 및 매칭 등급 표시
   
**예시 결과**:
```
🎯 '고윤정' 닮은 지원자 순위

👑 1위 - 지원자 C (92.3%) - 🔥 Perfect Match!
   2위 - 지원자 A (87.5%) - ✨ Great Match
   3위 - 지원자 E (82.1%) - ✨ Great Match
   4위 - 지원자 B (76.8%) - 👍 Good Match
   5위 - 지원자 D (68.2%) - 💫 Potential
```

#### 모드 2: 일반 유사 배우 탐색
1. **레퍼런스 배우 입력란 비우기** (빈 상태로 유지)
2. **지원자 사진 업로드**: 분석할 이미지 업로드
3. **Top-K 설정**: 슬라이더로 반환할 유사 배우 개수 설정 (1-10개)
4. **분석 시작**: 버튼 클릭
5. **결과 확인**: 
   - 각 지원자마다 Top-K 유사 배우 리스트 표시
   - "이 지원자는 고윤정(85%), 김태리(80%), 한지민(75%)과 닮았습니다"
...
- 대소문자 구분 없음, 공백 자동 제거
- 존재하지 않는 배우 입력 시 404 에러 반환 (명확한 오류 메시지 제공)

**결과 해석**:
- 점수 범위: 0.0 ~ 1.0 (코사인 유사도)
- 0.85 이상: 🔥 Perfect Match! (매우 유사)
- 0.75 ~ 0.85: ✨ Great Match (유사)
- 0.65 ~ 0.75: 👍 Good Match (적당히 유사)
- 0.65 미만: 💫 Potential (낮은 유사도)
...
#### 모드 1: 레퍼런스 배우 기반 지원자 랭킹
```http
POST /match-actors-batch?reference_actor=고윤정
Content-Type: multipart/form-data
  ...
  "items": [
    {
      ...
      "reference_actor": "고윤정",
      "similarity": 0.9234,
      "image_url": "http://localhost:8000/actors/고윤정/001.jpg"
    },
    {
  ...
  "items": [
    {
      ...
      "results": [
        ...
        },
        {
          "actor_name": "김태리",
          "similarity": 0.7892,
          "image_url": "http://localhost:8000/actors/김태리/001.jpg"
        }
      ]
    }
  ]
...
**Query Parameters**:
- `top_k` (int, optional): 반환할 결과 개수 (기본값: 3, 레퍼런스 모드에서는 무시됨)
- `reference_actor` (string, optional): 레퍼런스 배우 이름
  - **없으면**: 각 이미지마다 Top-K 유사 배우 반환
  - **있으면**: 각 이미지와 레퍼런스 배우 간의 유사도만 반환 (지원자 랭킹 모드)
Content-Type: multipart/form-data
